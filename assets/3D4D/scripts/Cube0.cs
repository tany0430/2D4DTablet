...
public class Cube0 : MonoBehaviour {
	...
	void Start () {
		...
		Cube0MH = GameObject.Find ("Cube0Mesh");
	...
	void Update () {
		...
		Cube0MH.gameObject.transform.eulerAngles = new Vector3(posX, posX, posX) * 90.0f;
		...
		cuberender.material.color = new Color (1, 0.95f, 0.95f, Alpha);
	...
	public void Cb3D(float posi)
	{
...

		{
			...
			vec(-0.5f - posi,  0.5f + posi , -0.5f -1),
			vec( 0.5f + posi,  0.5f + posi, -0.5f -1),
			vec( 0.5f + posi, -0.5f - posi, -0.5f -1),
			vec(-0.5f - posi, -0.5f - posi, -0.5f -1),

			vec(-0.5f ,  0.5f ,  0.5f -1),
			vec( 0.5f ,  0.5f ,  0.5f -1),
			vec( 0.5f , -0.5f ,  0.5f -1),
			vec(-0.5f , -0.5f ,  0.5f -1),
