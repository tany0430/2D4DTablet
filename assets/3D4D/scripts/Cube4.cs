...
public class Cube4 : MonoBehaviour {
	...
	void Start () {
		...
		Cube4MH = GameObject.Find ("Cube4Mesh");
	...
	void Update () {
		...
		Cube4MH.gameObject.transform.eulerAngles = new Vector3(posX, posX, posX) * 90.0f;
		...
		MeshRenderer cuberender = GetComponent<MeshRenderer> ();

		cuberender.material.color = new Color (0.95f, 1f, 0.95f, Alpha);
	...
	public void Cb3D(float posi)
	{
...

		{
			...
			vec(-0.5f+1 ,  0.5f  , -0.5f ),
			vec( 0.5f+1 ,  0.5f + posi , -0.5f - posi),
			vec( 0.5f+1, -0.5f - posi , -0.5f - posi),
			vec(-0.5f+1 , -0.5f , -0.5f ),
